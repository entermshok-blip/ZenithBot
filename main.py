...
class JudgeStates(StatesGroup):
    ...
    waiting_for_lang = State() # Новое состояние для выбора языка

# --- СЛОВАРЬ ЯЗЫКОВ (ТЕКСТЫ БОТА) ---
TEXTS = {
    'ru': {
...
def get_main_menu_kb(lang: str):
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=t['btn_reg'], web_app=WebAppInfo(url="https://benevolent-sawine-07ad42.netlify.app/"))],
        [InlineKeyboardButton(text=t['btn_judge'], callback_data="btn_judge")],
        [InlineKeyboardButton(text=t['btn_rules'], url="https://telegra.ph/REGLAMENT-ZENITH-ESPORTS-06-04")],
...
async def process_set_lang(callback: CallbackQuery, state: FSMContext):
    lang = callback.data.split("_")[-1] # Получит 'ru' или 'uz'
...
# Переключение языка из настроек (уже без состояния waiting_for_lang)
@router.callback_query(F.data.startswith("set_lang_"))
async def change_lang(callback: CallbackQuery, state: FSMContext):
    # Если юзер нажал это во время жалобы, отменяем жалобу
    await state.clear()
