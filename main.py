...
from aiohttp import web # ДОБАВЛЕНО: Импорт веб-сервера
import db
...
# --- ИЗМЕНЕНО: ЗАПУСК БОТА И ВЕБ-СЕРВЕРА ОДНОВРЕМЕННО ---
async def web_server_handler(request):
    # Просто отвечаем 200 OK на любой GET запрос
...
async def main():
    ...
    # 1. Создаем простое веб-приложение
    app = web.Application()
    app.router.add_get('/', web_server_handler)
    
    # 2. Узнаем порт из переменной окружения (для Render) или ставим 8000
    port = int(os.environ.get("PORT", 8000))
    
    # 3. Запускаем веб-сервер в фоне
    ...
    # 4. Запускаем polling бота
    # Используем gather, чтобы оба процесса работали одновременно
    await asyncio.gather(
        dp.start_polling(bot),
        asyncio.Event().wait() # Эта команда не дает программе завершиться
    )
